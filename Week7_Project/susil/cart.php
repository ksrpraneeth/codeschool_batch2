...
<body>
    <div class="container-fluid">
        ...
        <div class="row header2">
            ...
            <div class="col-md-1" style="margin-top:50px" id="logout"><a href="#" style="text-decoration: none;color:black;">LOGOUT</a></div>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-4">
                ...
                <input type="date" id="selectdate" name="date" class="form-control" placeholder="Select Date"
                    required="" aria-label="Use the arrow keys to pick a date" style="margin-top:17px;">
                <p id="date_alert" class="text-danger"></p>
            </div>

            <div class="col-md-4">
                <p>Delivery Time:</p>
                <select class="form-select" aria-label="Default select example" id="timeslot">
    ...
    <div class="container mt-5">
        ...
        <div class="row mt-3">
            <div class="offset-md-11 col-md-2"><button type="button" class="btn btn-primary" id="placeorder">Place
                    Order</button></div>
        </div>
    </div>
    <script>
...
<tr>
      ...
      `);
        ...
        // ajax call for delivery day
   

        // ajax call for placeorder
...
window.location.replace('product.php');
        ...
        // ajax call for delivery timetable


        $('#selectdate').change(function () {
            $('#date_alert').text('')
            $('#timeslot').empty()
            var c = $("#selectdate").val()
            var dt = new Date(c);
            console.log(dt.getDay() + 1)
            var formData = { deliveryDay: dt.getDay() + 1 }
            console.log(formData)
            $.ajax({
                type: 'POST', 
                url: 'getdayApi.php',
                ...
                success: function (response) {
                    let status = false
                    response = JSON.parse(response);
                    //  localStorage.setIteam('status',JSON.stringify(response));
                    console.log(response)
                    for (let i = 0; i < response.output.length; i++) {
                        if (response.output[i].status == 1) {
                            $('#timeslot').append(`<option value="${response.output[i].timeid}">${response.output[i].starttime}-${response.output[i].endtime}</option>`);
                            status = true
                        }
                    ...
                    if (!status) {
                        ...
                        $('#date_alert').text('Delivery not available on this day')
...
//     var fromdata={timeid:$('#timeslot option:selected').val()};

//     $.ajax({
//         type:"POST",
//         url:"timeupdateApi.php",
//         data:fromdata,
//         datatype:"JSON",
//         success:function(data){
//             data=JSON.parse(data)
//             if(data.status){
//                 window.alert("Delivery Slot is not available")
//             }
//             else{
//                 validate=true;
//             }
